...
export function ShareFileModal({ document: doc, onClose }) {
  ...
  const handleShare = async () => {
    ...
    try {
        ...
        toast.success(response.data.message || `File shared with ${email}`);
        setIsSharing(false);
        onClose();
    } catch (error) {
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
          ...
          <p className="text-sm text-gray-600 text-center mb-6">
            Are you sure you want to share <span className="font-medium">"{doc.filename}"</span> with <span className="font-medium">{email}</span>?
          </p>
          <div className="flex gap-3">
            ...
            <button
              onClick={handleShare}
              disabled={isSharing}
              className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all disabled:opacity-50"
            >
        ...
        <div className="p-6">
          ...
          <div className="flex items-start gap-3 p-4 bg-indigo-50 rounded-lg border border-indigo-200 mb-6">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-indigo-900">Secure Sharing</p>
              <p className="text-xs text-indigo-700 mt-1">Only secured files can be shared. The recipient will receive a notification with access to this file.</p>
            </div>
          </div>
